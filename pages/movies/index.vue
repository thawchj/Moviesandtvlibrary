<template>
  <div class="body-movies">
    ...
    >
      <div v-if="moviesArr && moviesArr.length > 0">
          ...
          <b-carousel-slide>
            <template #img>
              <img
                ...
                width="100%"
                :src="`https://www.themoviedb.org/t/p/w1920_and_h800_multi_faces/${moviesArr[index].backdrop_path}`"
                alt="image slot"
              />
              <div class="carousel-in">
                <div class="row">
                  <div class="col-8">
                    <h1 class="carousel-name">{{ moviesArr[index].title }}</h1>
                  </div>
                </div>
                <div class="my-3">
                  <nuxt-link :to="`/movies/${moviesArr[index].id}`">
...
export default {
  ...
  async asyncData({ $axios }) {
    const res = await $axios.$get(
      `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.API_KEY}&query=a&sort_by=popularity.desc&vote_count.gte=1000&page=1`
    )
    return {
      moviesArr: res.results,
    }
  },
  data() {
    return {
