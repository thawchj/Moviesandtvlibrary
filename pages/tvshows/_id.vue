<template>
    <div class="body">
        <div v-if="tvshow" class="body-tvshows container-fluid" :style="{ backgroundImage: `url(https://www.themoviedb.org/t/p/w1920_and_h800_multi_faces${tvshow.backdrop_path})` }">
            ...
            <div class="row offset-lg-1 d-flex align-items-center ">
                <div class="info-tvshows col-lg-4 p-5 col-md-12">
                    <img class="img-tvshows" :src="`https://www.themoviedb.org/t/p/w600_and_h900_bestv2${tvshow.poster_path}`" alt="" width="100%">
                </div>
                <div class="info-tvshows col-lg-6 col-md-12">
                    <h1 class="name-tvshows mt-5 mb-3">{{tvshow.name}}</h1>
                    <div class="time-tvshows my-2" >
                        <span class="text-time"><b-icon icon="calendar-event" class="mr-2" ></b-icon>{{tvshow.first_air_date}}</span>
                        <span class="text-time"><b-icon icon="clock" class="mr-2 " ></b-icon>{{tvshow.number_of_seasons}} seasons</span>
                        <span class="text-time">{{tvshow.number_of_episodes}} episodes</span>
                        <button class="add-button"  @click="addToList"><b-icon scale="1.5" icon="plus-circle" class="mr-2"></b-icon><span class="ml-2">ADD LIST</span></button>
                    </div>
                    ...
                    <div class="my-4 embed-responsive embed-responsive-16by9"> 
                        <iframe v-if="tvshow.videos && tvshow.videos.results && tvshow.videos.results[0]" width="560" height="315" :src="`https://www.youtube.com/embed/${tvshow.videos.results[0].key}?controls=0`" 
...
async asyncData({$axios , route }){
    let res = null
    try {
        res = await $axios.$get(`https://api.themoviedb.org/3/tv/${route.params.id}?api_key=${process.env.API_KEY}&append_to_response=videos,images`)
    }
    catch(e){
  ...
  methods: {
    addToList(){
        ...
        const checkhavetvshowselected = tvshowSelectedArr.some(ts => ts.idtvshow === item.idtvshow)
